...
본 연구는 도메인 갭을 극복하기 위해 **소프트웨어적 주파수 제어(Skip Filter)**와 **기하학적 복원(Hybrid Ellipse)**이라는 두 가지 상호 보완적인 접근법을 제안합니다.

### 🚀 가설 1: Skip Connection의 고주파 오염과 로우패스 필터 (Contribution 1)
...
### 🚀 가설 2: 파편화된 시멘틱의 기하학적 복원 (Contribution 2)
* **이론 및 방법:**
...
본 연구에서 제안한 두 모듈(Skip Filter, Hybrid Ellipse)은 데이터셋의 특성에 따라 서로 다른 기여도를 보였습니다. **Swirski** 데이터셋에서는 속눈썹 노이즈가 심해 **Skip Filter**가 핵심적인 역할을 했으며, **LPW** 데이터셋에서는 동공 파편화가 심해 **Hybrid Ellipse** 후처리가 더 큰 성능 향상을 견인했습니다. 결과적으로 두 모듈을 모두 결합했을 때 두 데이터셋 모두에서 최적의 성과를 거둘 수 있었습니다.
...
| 2 | Ellipse 후처리만 적용 | 0.6113 | **0.6304** | LPW에서 주도적 역할 |
| 3 | **Skip Filter 단독 적용 (σ=1.0)** | **0.6937** | 0.5576 | Swirski에서 주도적 역할 |
| 4 | **최종 파이프라인 (Filter + Ellipse)** | **0.7899** | **0.6376** | 최종 성과 🏆 |

#### 📊 테이블 4: Swirski 케이스별 최종 벤치마크 결과 (Baseline 대비)
순정 TransUNet 대비 최종 파이프라인의 케이스별 성능 비교입니다.

| Case | Baseline mIoU | Baseline mDice | Final mIoU | Final mDice |
|:---|:---:|:---:|:---:|:---:|
| p1-left | 0.6100 | 0.7514 | **0.8172** | **0.8956** |
| p1-right | 0.3504 | 0.5073 | **0.5629** | **0.6671** |
| p2-left | 0.6694 | 0.7765 | **0.8803** | **0.9335** |
| p2-right | 0.7025 | 0.8034 | **0.8990** | **0.9438** |
| **전체 평균** | 0.5831 | 0.7097 | **0.7899** | **0.8600** |

#### 📊 테이블 5: LPW 폴더별 최종 벤치마크 결과 (Baseline 대비)
순정 TransUNet 대비 최종 파이프라인의 폴더별 성능 비교입니다.

| Folder | Baseline mIoU | Baseline mDice | Final mIoU | Final mDice |
|:---:|:---:|:---:|:---:|:---:|
| 1 | 0.6270 | 0.7408 | **0.8536** | **0.9186** |
| 2 | 0.6991 | 0.7975 | **0.7655** | **0.8528** |
| 3 | 0.4319 | 0.5206 | **0.5841** | **0.6685** |
| 4 | 0.2457 | 0.3541 | **0.3883** | **0.4932** |
| 5 | 0.3651 | 0.4293 | **0.3917** | **0.4394** |
| 6 | 0.6394 | 0.7320 | **0.8068** | **0.8792** |
| 7 | 0.7230 | 0.8193 | 0.7126 | 0.8016 |
| 8 | 0.5280 | 0.6399 | **0.6870** | **0.7814** |
| 9 | 0.4712 | 0.5713 | **0.6519** | **0.7505** |
| 10 | 0.4835 | 0.5878 | **0.5498** | **0.6551** |
| 11 | 0.3763 | 0.4792 | **0.5285** | **0.6354** |
| 12 | 0.6061 | 0.6922 | **0.7547** | **0.8386** |
| 13 | 0.3687 | 0.4674 | **0.5087** | **0.6083** |
| 14 | 0.5319 | 0.6349 | **0.6986** | **0.7661** |
| 15 | 0.3953 | 0.4930 | **0.5475** | **0.6567** |
| 16 | 0.7239 | 0.8201 | **0.7741** | **0.8556** |
| 17 | 0.7608 | 0.8471 | 0.7184 | 0.8064 |
| 18 | 0.5504 | 0.6364 | **0.6868** | **0.7819** |
| 19 | 0.4130 | 0.5201 | **0.5232** | **0.6175** |
| 20 | 0.6112 | 0.7212 | **0.7655** | **0.8594** |
| 21 | 0.5732 | 0.6772 | **0.7182** | **0.8001** |
| 22 | 0.4342 | 0.5588 | **0.5561** | **0.6677** |
| **전체 평균** | 0.5254 | 0.6245 | **0.6448** | **0.7339** |
