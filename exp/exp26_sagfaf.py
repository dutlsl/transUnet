...
Fourier Adaptive Filtering (SAGFAF):
  Stage 1 (Input):   Fast inpainting on the resized 224x224 image (10x faster!).
  Stage 2 (Feature): SAGFAF on skip[0] features ONLY when OE=True.

Why SAGFAF is a Masterpiece of Signal Processing & Deep Learning Integration:
  - In overexposed pupil images, background noise (eyelashes, reflection borders) and
    pupil boundaries coexist in the same feature map.
  - A global frequency filter (like pure low-pass or pure high-pass) is a trade-off:
    low-pass smooths noise but blurs the pupil; high-pass sharpens boundaries but amplifies noise.
  - SAGFAF resolves this via Spatially-Varying, Self-Attention Guided Filtering:
    1. It computes a spatial Self-Attention map A(x, y) directly from the channel-wise
       activation magnitude of the skip features:
           S(x, y) = Mean_c( |F_c(x, y)| )
           A(x, y) = Sigmoid( gamma * (S - mean(S)) / std(S) )
    2. It computes a clean Fourier Low-Pass Filtered map F_LP (to smooth background noise).
    3. It computes a sharp Fourier High-Emphasis Filtered map F_HP (to sharpen pupil edges).
    4. It blends them dynamically in the spatial domain:
           F_adaptive(x, y) = A(x, y) * F_HP(x, y) + (1 - A(x, y)) * F_LP(x, y)
  - This training-free, non-iterative filter perfectly sharpens the pupil boundary
    while completely erasing the high-frequency eyelash and glare-border noise in the background!
"""
...
def inpaint_glare_resized(img_resized, bright_val=240, dilate_k=5, radius=5):
    ...
    # Telea inpainting on 224x224 is extremely fast (less than 1ms!)
...
# ── Stage 2: Spatially-Varying Self-Attention Guided Fourier Adaptive Filter ────
def sagfaf_filter(feat, d_lp_frac=0.20, d_hp_frac=0.40, hp_gain=0.6, gamma=2.0):
    """
    feat: [B, C, H, W]
    d_lp_frac: cutoff for Low-Pass Filter
    d_hp_frac: cutoff for High-Frequency Emphasis Filter
    ...
    # 2. Build Fourier Filter Masks
    yy = torch.arange(H, device=device).float().view(H, 1).expand(H, W)
    xx = torch.arange(W, device=device).float().view(1, W).expand(H, W)
    dist_sq = (yy - cy)**2 + (xx - cx)**2
    
    # Gaussian LPF mask
    d_lp = d_lp_frac * min(H, W)
    glp_mask = torch.exp(-dist_sq / (2 * (d_lp**2)))
    glp_mask_shifted = torch.fft.ifftshift(glp_mask).unsqueeze(0).unsqueeze(0)
    
    # Gaussian HFEF mask (boosts high frequencies, preserves low-pass flatly)
    ...
    # 3. Apply Filters in Fourier Domain
    feat_fft = torch.fft.fft2(feat)
    
    feat_fft_lp = feat_fft * glp_mask_shifted
    feat_fft_hp = feat_fft * ghfe_mask_shifted
    
    feat_lp = torch.fft.ifft2(feat_fft_lp).real
    feat_hp = torch.fft.ifft2(feat_fft_hp).real
    
    # 4. Spatially-Varying Blend guided by the Self-Attention Map
    feat_adaptive = attn_map * feat_hp + (1.0 - attn_map) * feat_lp
...
def patched_decoder(self, hidden_states, features=None):
    ...
    d_lp_frac = getattr(self, 'd_lp_frac', 0.20)
    ...
    if features is not None:
        ...
        # Stage 2: Spatially-varying SAGFAF on skip[0]
        if is_oe and len(features) > 0:
            features[0] = sagfaf_filter(features[0], 
                                        d_lp_frac=d_lp_frac, 
                                        d_hp_frac=d_hp_frac, 
                                        hp_gain=hp_gain, 
                                        gamma=gamma)
...
# ── Model factory ─────────────────────────────────────────────────────────────
def get_model(device, d_lp, d_hp, hp_gain, gamma):
...
def main():
    ...
    parser.add_argument('--d_lp', type=float, default=0.20, help='LPF cutoff fraction')
    ...
    print(f"==========================================================")
    print(f"🚀 SAGFAF Combined [LP={args.d_lp} HP={args.d_hp} gain={args.hp_gain} gamma={args.gamma}]")
    print(f"==========================================================")
    sys.stdout.flush()

    model      = get_model(device, args.d_lp, args.d_hp, args.hp_gain, args.gamma)
    ...
    for case_dir in case_dirs:
        ...
        for i, ff in enumerate(frame_files):
            ...
            # Stage 1 – Conditional fast inpaint on resized 224x224
    ...
    if total_iou:
        ...
        print(f"\n🏆 [SAGFAF LP={args.d_lp} HP={args.d_hp}] Total mIoU: {t:.4f}")
        sys.stdout.flush()
