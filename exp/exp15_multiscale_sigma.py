...
def patched_decoder_forward_multiscale(self, hidden_states, features=None):
    sigma0 = 1.0
    sigma1 = 0.8
    sigma2 = 0.3
    ...
    if features is not None:
        ...
        if len(features) > 2:
            k2 = int(4 * sigma2 + 0.5); k2 = k2 + 1 if k2 % 2 == 0 else k2
...
def get_model(device):
    ...
    model.decoder.forward = types.MethodType(patched_decoder_forward_multiscale, model.decoder)
...
def main():
    ...
    print(f"========================================")
    print(f"🚀 Multi-Scale Sigma (1.0, 0.8, 0.3) on {device}")
    print(f"========================================")
    sys.stdout.flush()
    ...
    if total_iou:
        ...
        print(f"\n🏆 [Total Multi-Scale Sigma] mIoU: {t_mIoU:.4f}")
        sys.stdout.flush()
