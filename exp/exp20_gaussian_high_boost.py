"""
exp20_gaussian_high_boost.py
──────────────────────────────────────────────────────────────────────────────
Two-Stage OE Correction with Fourier Gaussian High-Frequency Emphasis (GHFE):
  Stage 1 (Input):   Glare inpainting (Telea) on saturated pixels (>240).
  Stage 2 (Feature): Fourier-domain Gaussian High-Frequency Emphasis Filter (GHFEF)
                     on skip[0] features ONLY when OE=True.

Why Fourier Gaussian High-Frequency Emphasis (GHFE) is mathematically perfect here:
  1. Inpainting removes the bright glare but can leave the pupil boundary slightly
     smooth or blurry, which reduces segmentation sharpness.
  2. A High-Frequency Emphasis Filter (GHFEF) in the Fourier domain amplifies the
     edge gradients without destroying low-frequency global structure.
  3. The transfer function is:
         H_HFE(u, v) = a + b * (1 - exp(-D^2 / (2 * D0^2)))
     where:
         - a = 1.0 (preserves the base low-frequency features)
         - b = high-frequency gain (e.g. 0.5 to 1.5, to sharpen the boundaries)
         - D0 = cutoff frequency
  4. This selectively sharpens the reconstructed pupil-iris boundary in the feature
     space, boosting downstream localization and ellipse fitting accuracy!
"""
...
def gaussian_high_frequency_emphasis(feat, d0_frac=0.30, a=1.0, b=0.8):
    """
    feat: [B, C, H, W]
    H_HFE(u, v) = a + b * (1 - exp(-D^2 / (2 * D0^2)))
    """
    ...
    # 1. Create Gaussian High-Pass filter mask centered in frequency domain
    cy, cx = H // 2, W // 2
    d0 = d0_frac * min(H, W)
    ...
    # Gaussian High-Pass foundation: 1 - exp(-D^2 / (2 * D0^2))
    ghp_mask = 1.0 - torch.exp(-dist_sq / (2 * (d0**2)))
    
    # High-Frequency Emphasis mask: a + b * H_HP
    hfe_mask = a + b * ghp_mask
    
    # Shift mask to match raw 2D FFT output layout
    hfe_mask_shifted = torch.fft.ifftshift(hfe_mask)
    
    # 2. Perform 2D FFT, apply mask, and compute inverse FFT
...
def patched_decoder(self, hidden_states, features=None):
    ...
    d0_frac = getattr(self, 'd0_frac', 0.30)
    gain_b = getattr(self, 'gain_b', 0.8)
    ...
    if features is not None:
        ...
        # Stage-2: GHFEF on skip[0] ONLY when overexposed
...
def main():
    ...
    parser.add_argument('--d0_frac', type=float, default=0.30,
                        help='Gaussian FFT cut-off frequency fraction')
    parser.add_argument('--gain_b', type=float, default=0.8,
                        help='High-frequency amplification gain b')
    ...
    print(f"==========================================================")
    print(f"🚀 OE Combined [inpaint + GHFEF d0={args.d0_frac} b={args.gain_b}] | {device}")
    print(f"==========================================================")
    sys.stdout.flush()
    ...
    if total_iou:
        ...
        print(f"\n🏆 [GHFEF d0={args.d0_frac} b={args.gain_b}] Total mIoU: {t:.4f}")
        sys.stdout.flush()
