"""
exp25_lower_bright_val.py
──────────────────────────────────────────────────────────────────────────────
Two-Stage OE Correction with Deep Glare Threshold Optimization:
  Stage 1 (Input):   Inpaint all pixels > bright_val (160 to 190) to completely
                     remove the glare and its halo.
  Stage 2 (Feature): Baseline skip filters (Gaussian blur).

Why Lowering the Brightness Threshold is the Key to Peak Performance:
  - Saturated glare has a bright center (>240) and a soft halo slope (180 to 230).
  - If we only inpaint >240, the halo remains, which strongly confuses the network's
    receptive fields.
  - The pupil is extremely dark (<50) and the iris is mid-gray (80 to 140).
  - Therefore, lowering the inpaint threshold to 180 or 160 will safely capture
    the ENTIRE glare halo without touching the pupil/iris structure!
"""
...
# ── Stage 1: input-level glare inpaint with custom threshold ──────────────────
def inpaint_glare(img_gray, bright_val=180, dilate_k=5, radius=7):
...
def is_overexposed(img_gray, thresh=0.08, bright_val=180):
...
def main():
    ...
    parser.add_argument('--bright_val', type=int, default=180,
                        help='Brightness threshold to detect glare')
    parser.add_argument('--oe_thresh', type=float, default=0.08)
    args = parser.parse_args()

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"==========================================================")
    print(f"🚀 Lower Bright Val Combined [bright_val={args.bright_val}] | {device}")
    print(f"==========================================================")
    sys.stdout.flush()
    ...
    for case_dir in case_dirs:
        ...
        for i, ff in enumerate(frame_files):
            ...
            img_proc = inpaint_glare(img, bright_val=args.bright_val, dilate_k=5, radius=7) if oe else img
    ...
    if total_iou:
        ...
        print(f"\n🏆 [bright_val={args.bright_val}] Total mIoU: {t:.4f}")
        sys.stdout.flush()
