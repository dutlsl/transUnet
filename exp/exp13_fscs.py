...
def frequency_selective_channel_suppression(feature, beta=0.3):
    B, C, H, W = feature.shape
    feat_fft = torch.fft.fft2(feature)
    mag = torch.abs(feat_fft)

    total_energy = mag.sum(dim=(-2, -1)) + 1e-8

    cy, cx = H // 2, W // 2
    r_thresh = min(H, W) // 4
    yy = torch.arange(H, device=feature.device).view(H, 1).expand(H, W)
    xx = torch.arange(W, device=feature.device).view(1, W).expand(H, W)
    dist = ((yy - cy)**2 + (xx - cx)**2).float().sqrt()
    high_mask = (dist > r_thresh).float()

    high_mask_shifted = torch.fft.ifftshift(high_mask)
    high_energy = (mag * high_mask_shifted).sum(dim=(-2, -1))

    high_ratio = high_energy / total_energy

    gate = 1.0 - beta * high_ratio
    gate = gate.view(B, C, 1, 1)

    return feature * gate

def patched_decoder_forward_fscs(self, hidden_states, features=None):
    sigma0, sigma1 = 1.0, 0.5
    beta = getattr(self, 'fscs_beta', 0.3)
    ...
    if features is not None:
        ...
        if len(features) > 0:
            features[0] = frequency_selective_channel_suppression(features[0], beta)
        if len(features) > 1:
            features[1] = frequency_selective_channel_suppression(features[1], beta)
...
def get_model(device, beta):
    ...
    model.decoder.fscs_beta = beta
    model.decoder.forward = types.MethodType(patched_decoder_forward_fscs, model.decoder)
...
def main():
    ...
    parser.add_argument('--beta', type=float, default=0.2, help='FSCS Beta parameter')
    args = parser.parse_args()

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"========================================")
    print(f"🚀 FSCS (Frequency-Selective Channel Suppression, beta={args.beta}) on {device}")
    print(f"========================================")
    sys.stdout.flush()

    model = get_model(device, args.beta)
    ...
    if total_iou:
        ...
        print(f"\n🏆 [Total FSCS beta={args.beta}] mIoU: {t_mIoU:.4f}")
        sys.stdout.flush()
