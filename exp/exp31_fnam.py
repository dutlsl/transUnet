...
exp31_fnam.py
──────────────────────────────────────────────────────────────────────────────
Two-Stage OE Correction with Forced Neighborhood Attention Mask (FNAM)
& Raw-Scale Inpainting:
  Stage 1 (Input):   Precise glare inpainting on raw 640x480 images (bright_val=240,
                     dilate_k=5, radius=7).
  Stage 2 (Attention): Inject a Forced Neighborhood Attention Mask (FNAM) directly
                     into all self-attention layers of the ViT encoder.

Why FNAM is the Ultimate Masterstroke:
  - Adding a small positive bias (like CAAB) is easily overridden by the highly
    confident pre-trained QK dot-products inside the model.
  - FNAM guarantees local feature aggregation by mathematically masking out all
    patches outside a radius R of any glare patch:
        B[i, j] = -10000.0 if (patch i is in Glare AND dist(i, j) > R) else 0.0
  - By adding this mask before the softmax, we force 100% of the attention weights
    of featureless glare patches to be distributed purely among their immediate,
    clean spatial neighbors (iris boundaries), achieving perfect pupil shape reconstruction!
"""
...
# ── Stage 2: Forced Neighborhood Attention Mask (FNAM) Computation ────────────
def compute_fnam_mask(img_gray, bright_val=240, r_limit=2.0, device='cuda'):
    ...
    if flat_mask.sum() > 0:
        ...
        dist = np.sqrt(np.sum(diff**2, axis=-1)) # [196, 196]
        
        # Non-neighborhood elements are masked out with -10000.0
        mask_out = dist > r_limit
        bias = np.where(mask_out, -10000.0, 0.0).astype(np.float32)
        
        # Apply ONLY to glare query patches (rows)
        bias = bias * flat_mask[:, None]
...
def patched_attention_forward(self, hidden_states):
    ...
    # Forced Neighborhood Attention Mask (FNAM) injection
...
def main():
    ...
    parser.add_argument('--radius', type=float, default=2.0, help='FNAM neighborhood radius')
    ...
    print(f"==========================================================")
    print(f"🚀 FNAM Attention Mask [radius={args.radius}]")
    print(f"==========================================================")
    sys.stdout.flush()
    ...
    for case_dir in case_dirs:
        ...
        for i, ff in enumerate(frame_files):
            ...
            # Stage 2 – FNAM Attention Mask injection ONLY when OE=True
            if oe:
                bias = compute_fnam_mask(img, bright_val=args.bright_val, r_limit=args.radius, device=device)
    ...
    if total_iou:
        ...
        print(f"\n🏆 [FNAM radius={args.radius}] Total mIoU: {t:.4f}")
        sys.stdout.flush()
