...
TABLE_DIR = Path("./Swirski_tables/Final_Verification")
IMG_SIZE = 224
NUM_CLASSES = 4
PUPIL_CLASS_ID = 3
# =====================================================================
...
def patched_decoder_forward_tta(self, hidden_states, features=None):
    ...
    for i, decoder_block in enumerate(self.blocks):
        ...
        # [기여 2] FFT 게이트 적용 (최적 위치인 Position 2)
        if i == 2 and skip is not None and hasattr(self, 'fft_gate'):
...
def original_alignment_loss(gated_skip, vit_output):
    """기존의 코사인 유사도 기반 Loss"""
    gated_down = F.adaptive_avg_pool2d(gated_skip, vit_output.shape[-2:])
    skip_sam = gated_down.mean(dim=1, keepdim=True)
    vit_sam = vit_output.mean(dim=1, keepdim=True)
    
    B = skip_sam.shape[0]
    skip_flat = skip_sam.view(B, -1)
    vit_flat = vit_sam.view(B, -1)
    
    cos_sim = F.cosine_similarity(skip_flat, vit_flat, dim=1)
...
def main():
    ...
    csv_path = TABLE_DIR / f"final_pos2_r{args.radius}_lr{args.lr}_it{args.iterations}.csv"

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Starting Final Verification (Pos=2, Radius={args.radius}) on {device}")

    model = get_transunet_model(device)
    
    # Position 2 channels = 64
    channels = 64
    ...
    with open(csv_path, mode='w', newline='') as csv_file:
        ...
        for case_dir in sorted(case_dirs):
            ...
            for frame_file in frame_files:
                ...
                for _ in range(args.iterations):
                    ...
                    loss = original_alignment_loss(model.decoder.gated_skip, model.decoder.vit_output)
        ...
        if total_iou:
            ...
            print(f"\n[Total Final Pos=2] mIoU: {t_mIoU:.4f}")
