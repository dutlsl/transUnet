...
def patched_decoder_forward_phase_corr(self, hidden_states, features=None):
    sigma0 = 1.0
    sigma1 = 0.5
    if features is not None:
        features = list(features)
        # Contribution 1 (Gaussian Blur) is perfectly preserved
        if len(features) > 0:
    ...
    vit_output = x 
    alpha = getattr(self, 'alpha', 0.2)
    
    for i, decoder_block in enumerate(self.blocks):
        skip = features[i] if (features is not None and i < self.config.n_skip) else None
        
        # Contribution 2 (Phase Correction) added on top of Contribution 1
        if i == 0 and skip is not None and alpha > 0:
            skip_2d = skip.mean(dim=1) 
            vit_2d = vit_output.mean(dim=1)
            ...
            ratio = corrected_2d / (skip_2d + 1e-8)
...
def get_model(device, alpha):
    ...
    model.decoder.forward = types.MethodType(patched_decoder_forward_phase_corr, model.decoder)
...
def main():
    ...
    print(f"========================================")
    print(f"🚀 Phase Correction (Alpha={args.alpha}) on {device}")
    print(f"========================================")
    sys.stdout.flush()
    ...
    if total_iou:
        ...
        print(f"\n🏆 [Total Phase Correction α={args.alpha}] mIoU: {t_mIoU:.4f}")
        sys.stdout.flush()
