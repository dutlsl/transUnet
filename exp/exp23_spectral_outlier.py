"""
exp23_spectral_outlier.py
──────────────────────────────────────────────────────────────────────────────
Two-Stage OE Correction with Fourier Spectral Outlier Suppression (FSOS):
  Stage 1 (Input):   Glare inpainting (Telea, dilate_k=5, radius=7).
  Stage 2 (Feature): Fourier Spectral Outlier Suppression (FSOS) on skip[0]
                     features ONLY when OE=True.

Why Fourier Spectral Outlier Suppression (FSOS) is a breakthrough contribution:
  - High-intensity glare and sharp artifact boundaries (like eyelash grid-lines)
    manifest as "spectral outliers"—frequency domain spikes with exceptionally high
    amplitudes compared to the smooth pupil boundary components.
  - FSOS dynamically computes the mean and standard deviation of the feature's 2D
    FFT amplitude channel-wise:
        Threshold_c = Mean(Amp_c) + k * Std(Amp_c)
  - It identifies any frequency components exceeding this threshold and caps them,
    leaving the rest of the spectrum untouched.
  - This training-free, self-adaptive filtering perfectly removes the high-contrast
    residual glare-halo and eyelash noise from skip connections, dramatically
    improving the pupil localization robustness!
"""
...
# ── Stage 2: Fourier Spectral Outlier Suppression (FSOS) ─────────────────────
def fourier_spectral_outlier_suppression(feat, k=2.5):
    ...
    # 2. Compute channel-wise mean and std
    mean_amp = amp.mean(dim=(-2, -1), keepdim=True)
    std_amp = amp.std(dim=(-2, -1), keepdim=True)
    
    # 3. Identify and cap spectral outliers
    thresh = mean_amp + k * std_amp
...
def patched_decoder(self, hidden_states, features=None):
    ...
    if features is not None:
        ...
        # Stage-2: FSOS on skip[0] ONLY when overexposed
...
def main():
    ...
    parser.add_argument('--k_val', type=float, default=2.5,
                        help='Threshold multiplier k for outlier suppression')
    parser.add_argument('--oe_thresh', type=float, default=0.08)
    args = parser.parse_args()

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"==========================================================")
    print(f"🚀 FSOS Combined [inpaint + FSOS k={args.k_val}] | {device}")
    print(f"==========================================================")
    sys.stdout.flush()
    ...
    if total_iou:
        ...
        print(f"\n🏆 [FSOS k={args.k_val}] Total mIoU: {t:.4f}")
        sys.stdout.flush()
