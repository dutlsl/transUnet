"""
exp19_gaussian_fft.py
──────────────────────────────────────────────────────────────────────────────
Two-Stage OE Correction with Gaussian Fourier Low-Pass Filtering (GFLPF):
  Stage 1 (Input):   Glare inpainting (Telea) on saturated pixels (>240).
  Stage 2 (Feature): Frequency-domain Gaussian Low-Pass Filter (GLPF) on skip[0]
                     features ONLY when OE=True.

Why Gaussian Fourier Low-Pass (GFLPF) is mathematically superior to Circular LP:
  A hard circular cut-off filter in the frequency domain causes "Gibbs Ringing"
  artifacts in the spatial domain. These ringing ripples distort the fine boundaries
  of the pupil, degrading ellipse fitting.
  A Gaussian filter has a smooth frequency transition:
      H(u, v) = exp(-D^2 / (2 * D0^2))
  where D0 is the cutoff distance. This mathematically guarantees ZERO spatial
  ringing, cleanly suppressing high-frequency eyelash shadows and glare-halo noise
  while preserving the smooth, continuous pupil boundary.
"""
...
# ── Stage 2: Gaussian Fourier Low-Pass Filter (GFLPF) ───────────────────────
def gaussian_fft_lowpass(feat, d0_frac=0.40):
    """
    feat: [B, C, H, W]
    d0_frac: Gaussian cutoff frequency as a fraction of min(H, W).
    """
    B, C, H, W = feat.shape
    device = feat.device
    
    # 1. Create Gaussian Low-Pass filter mask in centered frequency domain
    cy, cx = H // 2, W // 2
    d0 = d0_frac * min(H, W)
    ...
    # H(u, v) = exp(-D^2 / (2 * D0^2))
    gaussian_mask = torch.exp(-dist_sq / (2 * (d0**2)))
    
    # Shift mask to align with unshifted 2D FFT output
    gaussian_mask_shifted = torch.fft.ifftshift(gaussian_mask)
    
    # 2. Perform 2D FFT, apply mask channel-wise, and compute inverse FFT
    feat_fft = torch.fft.fft2(feat)
    feat_fft_filtered = feat_fft * gaussian_mask_shifted.unsqueeze(0).unsqueeze(0)
...
def patched_decoder(self, hidden_states, features=None):
    ...
    d0_frac = getattr(self, 'd0_frac', 0.40)
    ...
    if features is not None:
        ...
        # Stage-2: GFLPF on skip[0] ONLY when overexposed
        if is_oe and d0_frac > 0 and len(features) > 0:
            features[0] = gaussian_fft_lowpass(features[0], d0_frac=d0_frac)
...
def main():
    ...
    parser.add_argument('--d0_frac', type=float, default=0.40,
                        help='Gaussian FFT low-pass radius fraction')
    ...
    print(f"==========================================================")
    print(f"🚀 OE Combined [inpaint + GFLPF d0_frac={args.d0_frac}] | {device}")
    print(f"==========================================================")
    sys.stdout.flush()

    model      = get_model(device, args.d0_frac)
    ...
    if total_iou:
        ...
        print(f"\n🏆 [GFLPF d0_frac={args.d0_frac}] Total mIoU: {t:.4f}")
        sys.stdout.flush()
