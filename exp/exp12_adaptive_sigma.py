...
def patched_vit_forward_adaptive(self, x):
    if x.size()[1] == 1:
        x = x.repeat(1,3,1,1)
    ...
    if attn_weights:
        ...
        mean_attn = last_attn.mean(dim=1) # [B, 196, 196]
        patch_importance = mean_attn.sum(dim=1) # [B, 196]
        patch_importance = patch_importance.view(-1, 1, 14, 14)
        
        b, c, h, w = patch_importance.shape
        p_min = patch_importance.view(b, -1).min(dim=1, keepdim=True)[0].view(b, c, 1, 1)
        p_max = patch_importance.view(b, -1).max(dim=1, keepdim=True)[0].view(b, c, 1, 1)
        importance_norm = (patch_importance - p_min) / (p_max - p_min + 1e-8)
        self.decoder.sigma_map = importance_norm
    else:
        self.decoder.sigma_map = None
        
    x = self.decoder(x, features)
    logits = self.segmentation_head(x)
    return logits

def patched_decoder_forward_adaptive(self, hidden_states, features=None):
    sigma0 = 1.0
    sigma1 = 0.5
    importance_norm = getattr(self, 'sigma_map', None)
    
    if features is not None:
        features = list(features)
        
        if importance_norm is not None and len(features) > 1:
            imp0 = F.interpolate(importance_norm, size=features[0].shape[-2:], mode='bilinear', align_corners=False)
            imp1 = F.interpolate(importance_norm, size=features[1].shape[-2:], mode='bilinear', align_corners=False)
            
            k0 = int(4 * sigma0 + 0.5); k0 = k0 + 1 if k0 % 2 == 0 else k0
            blurred0 = TF.gaussian_blur(features[0], kernel_size=[k0, k0], sigma=[sigma0, sigma0])
            
            k1 = int(4 * sigma1 + 0.5); k1 = k1 + 1 if k1 % 2 == 0 else k1
            blurred1 = TF.gaussian_blur(features[1], kernel_size=[k1, k1], sigma=[sigma1, sigma1])
            
            # Blend based on Attention Entropy (Contribution 2) + Preserve Contribution 1 (Blur)
            features[0] = features[0] * imp0 + blurred0 * (1.0 - imp0)
            features[1] = features[1] * imp1 + blurred1 * (1.0 - imp1)
        else:
            if len(features) > 0:
                k0 = int(4 * sigma0 + 0.5); k0 = k0 + 1 if k0 % 2 == 0 else k0
...
def get_model(device):
    ...
    model.forward = types.MethodType(patched_vit_forward_adaptive, model)
    model.decoder.forward = types.MethodType(patched_decoder_forward_adaptive, model.decoder)
...
def main():
    ...
    print(f"========================================")
    print(f"🚀 Adaptive Sigma (Attention Guided) on {device}")
    print(f"========================================")
    sys.stdout.flush()
    ...
    if total_iou:
        ...
        print(f"\n🏆 [Total Adaptive Sigma] mIoU: {t_mIoU:.4f}")
        sys.stdout.flush()
