"""
exp28_optimal_sagfee.py
──────────────────────────────────────────────────────────────────────────────
Two-Stage OE Correction with Spatially-Varying, Self-Attention Guided
Fourier Edge Emphasis (SAGFEE) & Raw-Scale Inpainting:
  Stage 1 (Input):   Inpaint glare on the RAW 640x480 image using the highly
                     optimized parameters (bright_val=240, dilate_k=5, radius=7)
                     to perfectly preserve the pupil boundary, then resize to 224x224.
  Stage 2 (Feature): Spatially-Varying, Self-Attention Guided Fourier Edge
                     Emphasis (SAGFEE) on skip[0] features in the decoder.

Why SAGFEE + Raw-Scale Inpainting is the Ultimate, Bulletproof Champion:
  - Performing inpainting on the resized 224x224 image with dilate_k=5 dilates the
    mask by 14 pixels in raw scale, swallowing the pupil itself and dropping IoU.
  - Doing inpainting on the raw 640x480 image ensures a surgical and precise glare removal.
  - The feature-level SAGFEE dynamically computes the Self-Attention map of the skip
    feature map:
        F_adaptive(x, y) = A(x, y) * F_HP(x, y) + (1 - A(x, y)) * F(x, y)
  - This selectively and beautifully sharpens pupil boundaries in high-activation regions
    while keeping background context perfectly clean and unaltered!
"""
...
# ── Stage 1: input-level glare inpaint on RAW image (precise scale!) ─────────
...
def main():
    ...
    print(f"==========================================================")
    print(f"🚀 SAGFEE Raw-Scale Combined [HP={args.d_hp} gain={args.hp_gain} gamma={args.gamma}]")
    print(f"==========================================================")
    sys.stdout.flush()
